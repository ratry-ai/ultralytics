...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        # 🔥 UPDATED: tambah 1 channel centerness
    ...
    def forward_head(self, x):
        ...
        centers = torch.cat(
            [self.cv_center[i](x[i]).view(bs, 1, -1) for i in range(self.nl)], dim=-1
        )

        return {"boxes": boxes, "scores": scores, "centerness": centers, "feats": x}
    ...
    def _inference(self, x):
        ...
        # 🔥 APPLY CENTERNESS (FCOS STYLE)
        scores = x["scores"].sigmoid()
        centers = x["centerness"].sigmoid()
        scores = scores * centers
